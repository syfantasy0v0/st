...
// Cookie session middleware will be set up in an async function
// app.use(setUserDataMiddleware); will be moved as well
...
// File uploads
// On Vercel, we can't write to the disk. Use memory storage for uploads.
const storage = multer.memoryStorage();
app.use(multer({ storage: storage, limits: { fieldSize: 500 * 1024 * 1024 } }).single('avatar'));
...
async function initializeServer() {
    ...
    const cookieSecret = await getCookieSecret(globalThis.DATA_ROOT);
