...
import * as fs from './vercel-storage.js';
...
export async function cleanUploads() {
    try {
        ...
        if (await fs.existsSync(uploadsPath)) {
            const uploads = await fs.readdirSync(uploadsPath);
...
export async function migrateUserData() {
    ...
    if (!await fs.existsSync(path.join(publicDirectory, 'characters'))) {
        return;
    }
    ...
    if (!await fs.existsSync(backupDirectory)) {
        await fs.mkdirSync(backupDirectory, { recursive: true });
    }
    ...
    for (const migration of dataMigrationMap) {
        ...
        try {
            if (!await fs.existsSync(migration.old)) {
            ...
            if (migration.file) {
                ...
                await fs.cpSync(migration.old, migration.new, { force: true });
                ...
                await fs.rmSync(migration.old, { recursive: true, force: true });
            } else {
                // Copy the directory to the new location
                await fs.cpSync(migration.old, migration.new, { recursive: true, force: true });
...
export async function migrateSystemPrompts() {
    ...
    for (const directory of directories) {
        try {
            ...
            if (await fs.existsSync(migrateMarker)) {
                continue;
            }
            const backupsPath = path.join(directory.backups, '_sysprompt');
            await fs.mkdirSync(backupsPath, { recursive: true });
            const defaultPrompts = await getDefaultSystemPrompts();
            const instucts = await fs.readdirSync(directory.instruct);
            let migratedPrompts = [];
            for (const instruct of instucts) {
                ...
                if (path.extname(instruct) === '.json' && !await fs.existsSync(sysPromptPath)) {
                    const instructContent = await fs.readFileSync(instructPath, 'utf8');
                    ...
                    if ('system_prompt' in instructData && 'name' in instructData) {
                        ...
                        await fs.cpSync(instructPath, backupPath, { force: true });
                        ...
                        await writeFileAtomicSync(instructPath, JSON.stringify(instructData, null, 4));
                    }
                }
            }
...
export async function getCookieSecret(dataRoot) {
    ...
    if (await fs.existsSync(cookieSecretPath)) {
        const stat = await fs.statSync(cookieSecretPath);
        if (stat.size > 0) {
            return await fs.readFileSync(cookieSecretPath, 'utf8');
        }
    }
...
export async function getUserAvatar(handle) {
    try {
        ...
        const settingsContent = await fs.existsSync(pathToSettings) ? await fs.readFileSync(pathToSettings, 'utf8') : '{}';
        ...
        if (!await fs.existsSync(avatarPath)) {
            return PUBLIC_USER_AVATAR;
        }
        const mimeType = mime.lookup(avatarPath);
        const base64Content = await fs.readFileSync(avatarPath, 'base64');
...
function createRouteHandler(directoryFn) {
    return async (req, res) => {
        try {
            ...
            const exists = await fs.existsSync(path.join(directory, filePath));
            if (!exists) {
...
function createExtensionsRouteHandler(directoryFn) {
    return async (req, res) => {
        try {
            ...
            const existsLocal = await fs.existsSync(path.join(directory, filePath));
            if (existsLocal) {
                return res.sendFile(filePath, { root: directory });
            }

            const existsGlobal = await fs.existsSync(path.join(PUBLIC_DIRECTORIES.globalExtensions, filePath));
            if (existsGlobal) {
